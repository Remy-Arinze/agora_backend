...
© ${new Date().getFullYear()} Agora school space. All rights reserved.`;
  ...
  /**
   * Send OTP email for in-app password change verification
   */
  async sendPasswordChangeOtpEmail(
    email: string,
    name: string,
    otpCode: string,
  ): Promise<void> {
    ...
    this.logger.log(`Password change OTP email sent to ${email}`);
  }

  /**
   * Send OTP email for forgot-password (reset) verification
   */
